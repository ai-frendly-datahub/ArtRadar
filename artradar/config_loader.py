...
def _dict_items(value: object) -> list[dict[str, object]]:
    ...
    for item in cast(list[object], value):
        if isinstance(item, dict):
            ...
            items.append({str(key): entry for key, entry in item_dict.items()})
    return items
...
def _resolve_env_refs(value: object) -> object:
    if isinstance(value, str):
        ...
        return result
    if isinstance(value, dict):
        return {key: _resolve_env_refs(entry) for key, entry in value.items()}
    if isinstance(value, list):
    ...
    if isinstance(email_raw, dict):
        ...
        try:
            ...
            email_settings = EmailSettings(
                ...
                to_addresses=[
                    str(address)
                    for address in cast(list[object], email_dict.get("to_addresses", []))
