...
config_loader = import_module("artradar.config_loader")
collector_module = import_module("artradar.collector")
analyzer_module = import_module("artradar.analyzer")
validators_module = import_module("artradar.common.validators")
raw_logger_module = import_module("artradar.raw_logger")
...
def _send_notifications(
    *,
    ...
    notifier_module = import_module("artradar.notifier")
    composite_notifier_cls = notifier_module.CompositeNotifier
    email_notifier_cls = notifier_module.EmailNotifier
    notification_payload_cls = notifier_module.NotificationPayload
    webhook_notifier_cls = notifier_module.WebhookNotifier

    payload = notification_payload_cls(
    ...
    if webhook_url:
        notifiers.append(webhook_notifier_cls(url=webhook_url))

    if notifiers:
        composite = composite_notifier_cls(notifiers)
...
if __name__ == "__main__":
    ...
    _ = run(
        ...
        browser_source_limit=_to_optional_int(args.get("browser_source_limit")),
